...
/**
 * Histórico: sessões concluídas + sessões CANCELADAS que registraram séries
 * (indicadas como "Cancelado" na UI; nunca contam em métricas). Canceladas
 * sem nenhuma série não aparecem — não há o que consultar nelas.
 */
export async function getSessionsHistory(
  ...
  const { data, error } = await supabase
    .from('workout_sessions')
    .select('*, workout:workouts(*), set_logs(count)')
    .eq('user_id', userId)
    .or('finished_at.not.is.null,cancelled_at.not.is.null')
    .order('started_at', { ascending: false })
    .limit(limit)

  if (error) throw error
  const rows = (data ?? []) as unknown as Array<
    SessionWithWorkoutRow & { set_logs: Array<{ count: number }> }
  >
  return rows
    .filter(
      (session) =>
        session.finished_at != null || (session.set_logs?.[0]?.count ?? 0) > 0
    )
    .map((row) => {
      const { set_logs: _ignored, ...session } = row
      void _ignored
      return session as SessionWithWorkoutRow
    })
}
...
export type StartOrResumeResult =
  ...
  | { kind: 'resumed'; sessionId: string; workoutId: string; sameWorkout: boolean }

/**
 * Único caminho para iniciar treino: nunca cria segunda sessão ativa.
 * Se já existe sessão ativa, retorna 'resumed' para o app abrir/decidir.
 * Em corrida de inserção (índice único parcial → 23505), busca e retorna a
 * sessão vencedora em vez de falhar.
 */
    ...
    if (insertError.code === '23505') {
      ...
      if (winner) {
        return {
          kind: 'resumed',
          sessionId: winner.id,
          workoutId: winner.workout_id,
          sameWorkout: winner.workout_id === workoutId,
        }
      }
    }
    throw insertError
...
/**
 * Cancelamento LÓGICO (P0.2): marca cancelled_at em vez de apagar. Todas as
 * séries registradas (set_logs) são preservadas; a sessão sai das métricas
 * (que filtram finished_at is not null) mas continua consultável.
 */
export async function cancelSessionLogically(
  supabase: SupabaseDB,
