...
A sessão ativa, séries e timers baseados em timestamp são persistidos em `localStorage`. Escritas usam UUIDs gerados no cliente e uma fila idempotente; ao voltar a conexão, sessões são sincronizadas antes das séries. O indicador na execução mostra itens pendentes. Isso protege uma sessão já aberta contra atualização/instabilidade; uma abertura totalmente offline ainda depende de cache de navegação/PWA, que não existia na arquitetura atual.
