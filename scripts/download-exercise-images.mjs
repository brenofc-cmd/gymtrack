...
const results = await Promise.all(
  exerciseIds.map(async (id) => {
    const response = await fetch(`${SOURCE_ROOT}/${encodeURIComponent(id)}/0.jpg`)
    if (!response.ok) {
      throw new Error(`Falha ao baixar ${id}: HTTP ${response.status}`)
    }

    const bytes = new Uint8Array(await response.arrayBuffer())
    await writeFile(join(OUTPUT_DIR, `${id}.jpg`), bytes)
    return { id, bytes: bytes.byteLength }
  })
)

const totalBytes = results.reduce((total, result) => total + result.bytes, 0)
console.log(`Baixadas ${results.length} imagens (${(totalBytes / 1024 / 1024).toFixed(2)} MB).`)
