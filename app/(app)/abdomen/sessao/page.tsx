...
export default async function AbdomenSessaoPage() {
  ...
  const [catalog, state] = await Promise.all([getCoreCatalog(supabase), getCoreUserState(supabase, user.id)])
  ...
  return <CoreSessionClient userId={user.id} day={day} exercises={exercises} adaptationWeek={week} existingSession={existingSession} />
}
