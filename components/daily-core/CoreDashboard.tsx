...
import { Activity, ArrowRight, CalendarDays, Check, ChevronRight, CircleGauge, Dumbbell, HeartPulse, Moon, Settings, ShieldCheck, Sparkles } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { toast } from 'sonner'
import { createClient } from '@/lib/supabase/client'
import { localDateISO, nextSession, streakStats, type CoreExercisePlan, type CoreExerciseWithVariations } from '@/lib/daily-core/logic'
import type {
  DailyCoreDayRow,
  DailyCorePainCheck,
  DailyCorePainLogRow,
...
export function CoreDashboard({ userId, today, allDays, plan, exerciseCatalog, sessions, sets, pain, progressions, adaptationWeek }: CoreDashboardProps) {
  ...
  return (
    <div className="space-y-4">
      <section className="relative overflow-hidden rounded-[22px] border border-border bg-card p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-[11px]">
          ...
          <span className="rounded-full border border-input bg-secondary px-2.5 py-1">{todaySession?.status === 'concluido' ? 'Concluído' : todaySession?.status === 'em_andamento' ? 'Em andamento' : 'Não iniciado'}</span>
        </div>
        {!today.is_rest && todaySession?.status !== 'concluido' && (
          <Link href="/abdomen/sessao" className="mt-5 flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-primary text-sm font-bold text-primary-foreground">
            {todaySession?.status === 'em_andamento' ? 'Continuar rotina' : 'Iniciar rotina'} <ArrowRight className="size-4" />
          </Link>
        )}
